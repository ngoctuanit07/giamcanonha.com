...
class ShopOrderController extends Controller
{
    ...
    protected function grid($keyword)
    {
        ...
        return Admin::grid(Orderhome::class, function (Grid $grid) use ($keyword) {
            $grid->id('ID')->sortable();
			...
			$grid->sanpham('Sản Phẩm')->display(function ($sanpham) {
				return empty($sanpham) ? 'N/A' : '<div style="max-width:150px; overflow:auto;word-wrap: break-word;">' . $sanpham . '</div>';
			});
			$grid->soluong('Số lượng')->display(function ($soluong) {
				return empty($soluong) ? 'N/A' : '<div style="max-width:150px; overflow:auto;word-wrap: break-word;">' . $soluong . '</div>';
			});
			$grid->fullname('Họ và Tên')->display(function ($fullname) {
				return empty($fullname) ? 'N/A' : '<div style="max-width:150px; overflow:auto;word-wrap: break-word;">' . $fullname . '</div>';
			});
			$grid->diachi('Địa chỉ')->display(function ($diachi) {
				return empty($diachi) ? 'N/A' : '<div style="max-width:150px; overflow:auto;word-wrap: break-word;">' . $diachi . '</div>';
			});
			$grid->dienthoai('Điện Thoại')->display(function ($dienthoai) {
				return empty($dienthoai) ? 'N/A' : '<div style="max-width:150px; overflow:auto;word-wrap: break-word;">' . $dienthoai . '</div>';
			});
			$grid->ghichu('Ghi Chú')->display(function ($ghichu) {
				return empty($ghichu) ? 'N/A' : '<div style="max-width:150px; overflow:auto;word-wrap: break-word;">' . $ghichu . '</div>';
			});
